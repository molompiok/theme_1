...
async function startServer() {
  ...
  const loadMonitoring = new LoadMonitorService({
    ...
    logger: logger,
    serviceId: THEME_ID || 'theme_1-ID-in-Server-DB',
    serviceType: 'theme',
  });

  loadMonitoring.startMonitoring()
  const shutdown = async () => {
    console.log(`[SERVICE Server ${THEME_NAME}:${THEME_ID}] Arrêt demandé...`);
    server.close(async () => {
      console.log(`[SERVICE Server ${THEME_NAME}:${THEME_ID}] Serveur HTTP fermé.`);
      await closeQueue(); // Fermer la connexion BullMQ/Redis
      process.exit(0);
    });
    // Forcer la fermeture après un délai si le serveur ne se ferme pas
    setTimeout(async () => {
      console.error(`[SERVICE Server ${THEME_NAME}:${THEME_ID}] Arrêt forcé après timeout.`);
      await closeQueue();
      process.exit(1);
