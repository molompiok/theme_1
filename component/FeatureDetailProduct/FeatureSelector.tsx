...
export const FeatureSelector: React.FC<FeatureSelectorProps> = ({ features, product_id, feature_name }) => {
    ...
    return (
      <div className="p-0.5 pl-2" role="group" aria-label={`Options pour ${feature_name}`}>
              ...
              >
                {validFeatureValues.map((value) => {
                  const isSelected = selections
                    .get(product_id)
                    ?.get(feature.id)?.valueFeature === value.id;
                  return (
                    <ValueComponent
                      key={value.id}
                      value={value}
