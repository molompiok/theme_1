    ...
    <div className="container mx-auto min-h-dvh bg-white py-6 px-4 font-primary">
      <div
        className={`sticky inset-x-0 bg-white border-gray-200 transition-all w-full duration-300 py-5
           ${
             isScrolled
               ? "border-b mt-0 top-11 sm:top-15 z-40"
               : "top-0 mt-7 z-40"
           }`}
        style={{
        ...
        >
          {isFetchingNextPage ? (
            ...
            <p className="text-gray-500 text-sm">
              Tous les favoris ont été chargés
            </p>
          )}
        </div>
      )}
...
function ProductCard({ favorite }: { favorite: ProductFavorite | undefined }) {
  const { data: feature, status } = useQuery({
    queryKey: ["get_features_with_values", favorite?.product.default_feature_id],
    queryFn: () =>
      get_features_with_values({ feature_id: favorite?.product.default_feature_id }),
    enabled: !!favorite?.product.default_feature_id,
  });
      ...
      <div className="p-2 sm:p-3">
        ...
        <div className="flex items-center justify-between">
          <DisplayPrice
            ...
            barred_price={favorite?.product.barred_price ?? undefined}
          />
