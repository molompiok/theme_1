...
export const PhoneNumbers: React.FC<PhoneNumbersProps> = ({ maxItems = 3 }) => {
  ...
  const phoneNumbersFromStore = useAuthStore(
    (state) => state.user?.phone_numbers
  );
  const fetchUser = useAuthStore((state) => state.fetchUser);

  const normalizePhoneNumberFromApi = useCallback(
    (apiPhone: UserPhoneFromApi): UserPhone | null => {
      ...
      if (apiPhone.country_code) {
        ...
        country = countries.find(
          (c) =>
            c.iso === isoPart || (dialCodePart && c.dialCode === dialCodePart)
        );
      }
      ...
      if (!country) {
        ...
        country =
          countries.find((c) =>
            apiPhone.phone_number.startsWith("+" + c.dialCode)
          ) ||
  ...
  useEffect(() => {
    ...
    if (showAddForm) {
      setAddFormValue(
        IMask.pipe(`+${addFormCountry.dialCode}`, {
  ...
  const createUserPhoneMutation = useMutation({
    ...
    onError: (error: any) => {
      setAddFormError(
        error?.response?.data?.message || "Erreur lors de l'ajout. Réessayez."
      );
    },
  });
  ...
  const updateUserPhoneMutation = useMutation({
    ...
    onError: (error: any) => {
      setEditFormError(
        error?.response?.data?.message || "Erreur de mise à jour. Réessayez."
      );
    },
  });
  ...
  const isValidPhoneNumber = (
    ...
    return (
      rawDigits.startsWith(country.dialCode) &&
      rawDigits.length - country.dialCode.length === country.maxLength
    );
  };
  ...
  const handleAddFormCountryChange = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    setAddFormValue(
      IMask.pipe(`+${newCountry.dialCode}`, { mask: newCountry.mask } as any)
    );
    setAddFormError(null);
  };
  ...
  const handleAddSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (
      !addFormValue.trim() ||
      !isValidPhoneNumber(addFormValue, addFormCountry)
    ) {
      setAddFormError(
        `Format invalide. Exemple: ${addFormCountry.mask.replace("0", "#")}`
      );
      return;
    }
  ...
  const handleEditStart = (phone: UserPhone) => {
    const country =
      countries.find((c) => c.iso === phone.country_iso) || countries[0];
    setEditState({
      id: phone.id,
      country: country,
  ...
  const handleEditSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!editState || updateUserPhoneMutation.isPending || editFormError)
      return;

    if (
      !editState.value.trim() ||
      !isValidPhoneNumber(editState.value, editState.country)
    ) {
      setEditFormError(
        `Format invalide. Exemple: ${editState.country.mask.replace("0", "#")}`
      );
      return;
    }
    const normalizedNumber = getNormalizedPhoneNumber(
    ...
    if (
      numbers.some(
        (n) => n.phone_number === normalizedNumber && n.id !== editState.id
      )
    ) {
    ...
    <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                      ...
                      <div className="md:col-span-3">
                        ...
                        <IMaskInput
                          ...
                          autoFocus
                          disabled={updateUserPhoneMutation.isPending}
                          placeholder={`Ex: ${editState.country.mask.replace(
                            "0",
                            "#"
                          )}`}
      ...
      {showAddForm && numbers.length < maxItems && (
        <div className="relative group">
          <div className="absolute inset-0 bg-gradient-to-r from-emerald-100/30 to-blue-50/20 rounded-2xl blur-xl"></div>

          <div className="relative bg-white/90 backdrop-blur-xl border border-slate-200/60 rounded-2xl p-6 shadow-xl">
            <form
              ref={addFormRef}
                ...
                <div className="md:col-span-3">
                  ...
                  <IMaskInput
                    ...
                    onAccept={(value, maskRef) =>
                      handleAddFormValueChange(value)
                    }
                    disabled={createUserPhoneMutation.isPending}
