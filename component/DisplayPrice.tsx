...
export function DisplayPrice({
  ...
  return (
    <div className="flex font-secondary justify-start list-product-breakpoint-4:items-center gap-1  items-start list-product-breakpoint-4:flex-row ">
      ...
      {barred_price && (
        <h1 className="line-through font-light text-[.8rem] text-black/80  whitespace-nowrap list-product-breakpoint-4:block">
...
export function DisplayPriceDetail({
  price,
  currency,
  barred_price,
}: {
...
export function DisplayPriceItemCart({
  product,
}: {
  ...
  return (
    <h1 className=" whitespace-nowrap font-light text-clamp-base text-green-900">
      {formatPrice(
        itemInPanier ? itemInPanier.nbr * itemInPanier.product.price : 0
      )}{" "}
      {itemInPanier?.product.currency || ""}
    </h1>
  );
}
